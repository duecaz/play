...
export class TildesEditor extends BaseEditor {
  get title()    { return 'Tildes' }
  get subtitle() { return 'Escribe el texto correcto — el sistema quita las tildes y genera el ejercicio' }
    ...
    return `
      ...
      <section class="editor-section">
        ...
        <p class="section-hint">
          ...
          El sistema quitará las tildes (á→a, é→e, í→i, ó→o, ú→u) y marcará cada vocal como zona para el alumno.
        </p>
        <div class="mb-3">
  ...
  _updatePreview() {
    ...
    preview.innerHTML =
      `<p class="preview-label">Vista previa — lo que verá el alumno (${zones} tilde${zones !== 1 ? 's' : ''}):</p>` +
  ...
  validate() {
    ...
    if (!correct)
      ...
      for (let i = 0; i < correct.length; i++) if (original[i] !== correct[i]) zones++
      if (zones === 0)
        errors.push('El texto no tiene tildes — escribe el texto con las tildes correctas.')
    }
    return errors
  }
  ...
  buildActivity() {
    ...
    return {
      ...
      presentation: { skin, layout: 'center', sound: false, showTimer: true, showScore: true, teams: false, scoreMode: scoreOnly ? 'points' : 'full' }
    }
  }
}
